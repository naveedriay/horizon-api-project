@api @basketGB @hybridBasketGB @A2B
Feature: Hybrid Basket Api - Dev
  ...
  @sanity @regression @BASKET-247 @FULFIL-2212
  Scenario: Verify Creating only ATG basket by calling Hybrid Basket using basketType=ATG
    Given I create a new basket api request with following headers
      | Content-Type | application/json |
    And I add a json payload using filename basket/atg_basket_update.json replacing values
      | items[0].quantity | 32          |
    When I send a POST request to v2/basket?basketType=ATG endpoint
    Then The response status code should be 200
    And make sure there are 2 products in the basket
    And the following items contains correct status
      | 001388 | ADDED_MAXQUANTITY |
      | 060188 | ADDED             |
    And make sure following attributes exist within response json
      | atgSuccess                                 | true       |
      | basket.sessionInfo.JSESSIONID              | [NOTEMPTY] |
      | basket.sessionInfo.NBTY_BASKETITEMS_COOKIE | [NOTEMPTY] |
      | basket.sessionInfo.NBTY_ORDERID_COOKIE     | [NOTEMPTY] |
  ...
  @sanity @regression @BASKET-247 @FULFIL-2212
  Scenario: Verify Creating only Hybrid basket by calling Hybrid Basket using basketType=HBI
    Given I create a new basket api request with following headers
      | Content-Type | application/json |
    And I add a json payload using the file basket/hybrid/hybrid_basket_payload.json
    When I send a POST request to v2/basket?basketType=HBI endpoint
    Then The response status code should be 200
    And make sure there are 2 products in the basket
    And the following items contains correct status
      | 048791 | ADDED  |
...
#   Horizon Basket throws a 404
  ...
  Scenario: Verify applying valid ORDER LEVEL Voucher code to an existing Hybrid basket using PUT
    ...
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a PUT request to v2/basket/{basketUUID}/voucher/DIGITAL endpoint
    Then The response status code should be 200
    And make sure there are 2 products in the basket
    And make sure items total collectively make the basket total
    And make sure items subtotal collectively make the basket subtotal
    ...
    And make sure following attributes exist within response json
      ...
      | basket.coupons[0].alias         | DIGITAL  |
      | basket.coupons[0].description   | 10% Off - Digital Analytics Test  |
      | basket.promotions.promotion     | [Buy one, Get one Half Price, 10% Off - Digital Analytics Test]|
...
#    And make sure items total collectively make the basket total
  ...
  Scenario: Verify testing 25% staff discount applied to Hybrid basket ( No Promo Items)
    ...
    And make sure following attributes exist within response json
      ...
      | basket.total                 | 8.98   |
      | basket.promotions.promotion  | [Scenario 1d: 25% off Staff Discount, Buy One Get One 1/2 Price]|

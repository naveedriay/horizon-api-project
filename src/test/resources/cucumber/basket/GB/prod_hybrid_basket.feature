@api @prodBasketGB @prodHybridGB
Feature: Test Prod Hybrid Basket Api

  @regression @BASKET-247 @prod_basket_sanity
...
#  Currently there are no SBP items in Prod. Plz check later and update basket payload
  @regression @FULFIL-1677 @SBPItems @ignore
  ...
  @regression @BASKET-248 @BASKET-306 @FULFIL-1402 @Subscribe @prod_basket_sanity
...
#   Horizon Basket throws a 404
  @negative @BASKET-247 @prod_basket_sanity
  ...
  @vouchers @regression @BASKET-379 @prod_basket_sanity
  Scenario: Verify applying valid ORDER LEVEL Voucher code to an existing Hybrid basket using PUT
    ...
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a PUT request to v2/basket/{basketUUID}/voucher/PARTY-201810 endpoint
    Then The response status code should be 200
    And make sure following attributes exist within response json
      | responseStatus.result.status    | Applied                                                                                 |
      | responseStatus.result.voucherId | PARTY-201810                                                                            |
      | basket.coupons[0].description   | Happy Birthday! Here's your 20% off!                                                    |
      | basket.promotions.promotion     | [Buy One Get One 1/2 Price, Happy Birthday! Here's your 20% off!, 2 for £7 Mix & Match] |
  ...
  Scenario: Verify applying valid ITEM LEVEL Voucher code to an existing Hybrid basket using PUT
    ...
    And make sure following attributes exist within response json
      ...
      | basket.promotions.promotion         | [Buy One Get One 1/2 Price, 2 for £7 Mix & Match, 10% off Vegan] |

  @vouchers @negative @BASKET-379 @prod_basket_sanity
  ...
  Scenario: Verify Removing existing Voucher Code from a Hybrid basket using DELETE
    ...
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a DELETE request to v2/basket/{basketUUID}/voucher/PARTY-201810 endpoint
    Then The response status code should be 200
    ...
    And make sure following attributes exist within response json
      ...
      | responseStatus.result.voucherId | PARTY-201810                |
      | basket.promotions.promotion     | [Buy One Get One 1/2 Price, 2 for £7 Mix & Match]|
    And make sure following attributes doesnot exist within response json
      | basket.coupons |  |

  @vouchers @BASKET-379 @prod_basket_sanity

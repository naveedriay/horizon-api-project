@api @IEbasket @atgbasketIE
Feature: Test IE ATG Basket Api
  ...
  Scenario: Verify creating new ATG GBP Basket with valid payload json
    ...
    And I add a json payload using filename basket/IE_payloads/atg_basket_payload_IE.json replacing values
...
#      | basket.items[0].stock_promotion_threshold.enable| true              |
  ...
  Scenario: Verify ATG Basket's total after adding all types of promotion items
    Given I have successfully added items to ATG Basket using basket/IE_payloads/promotion_item_payload_IE.json
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a GET request to v1/basket endpoint
    ...
    And currently basket contains following items
      | skuId  | item_quantity | item_name                                    |
      | 073370 | 2             | Naturtint Permanent Hair Colour 1N Ebony Black             |
      | 003969 | 2             | Dr Organic Moroccan Argan Oil Day Cream 50ml        |
    And make sure that basket.total is less than basket.subtotal
    And make sure following attributes exist within response json
    #  | basket.currency                | EUR                            |
    #  | basket.locale                  | en-IE                            |
      | basket.promotions[0].promotion | Buy One Get One For a Penny           |
      | basket.promotions[1].promotion | Free Standard delivery on £25       |
  ...
  Scenario: Verify ATG Basket's total after adding all types of non promo items
    Given I have successfully added items to ATG Basket using basket/IE_payloads/nonpromo_item_payload_IE.json
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a GET request to v1/basket endpoint
  ...
  Scenario: Verify adding valid Voucher Code to existing ATG basket using PUT request
    ...
    And I create a new basket api request with following headers
      | Content-Type | application/json |
      | Cookie       | [JSession-ID]    |
    When I send a PUT request to v1/basket/voucher/VEGETARIAN10 endpoint
    Then The response status code should be 200
    And make sure there are 3 products in the basket
    And make sure that basket.total is less than basket.subtotal
    And make sure following attributes exist within response json
      ...
      | basket.coupons[0].code          | [number]    |
      | basket.coupons[0].description   | Vegetarian 10%  |
      | basket.promotions[0].promotion    | Vegetarian 10%|
  ...
  Scenario: Verify Removing existing Voucher Code from a ATG basket using DEL request
    ...
    And make sure following attributes exist within response json
      ...
      | responseStatus.result.voucherId | VEGETARIAN10|
      | basket.promotions[0].promotion    | Free Standard delivery on £25 |
    And make sure following attributes doesnot exist within response json
      | basket.coupons |                |
